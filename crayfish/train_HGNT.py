"""
小龙虾基因表达预测 - HG-NT (Multi-Graph Concat)
...
# 单次训练运行（固定种子）
# =================================================================
def run_single_seed(data_dict, args, seed):
    """使用固定种子进行单次训练"""
    print(f"\n{'=' * 50}")
    print(f"🎲 种子: {seed}")
    print(f"{'=' * 50}")
...
# 多种子训练
# =================================================================
def train_multi_seed(data_dict, args):
    """使用多个种子进行训练"""
    seeds = args.seeds if args.seeds else DEFAULT_SEEDS
    print(f"\n{'=' * 70}")
    print(f"🚀 多种子训练 - 小龙虾 HG-NT")
    print(f"种子列表: {seeds}")
    print(f"{'=' * 70}")

    all_results = []
    for seed in seeds:
        result = run_single_seed(data_dict, args, seed)
        if result:
    ...
    results_file = os.path.join(args.output_dir, 'crayfish_hgnt_seed_results.csv')
    ...
    with open(os.path.join(args.output_dir, 'crayfish_hgnt_summary.json'), 'w') as f:
        json.dump(summary, f, indent=2)

    print(f"💾 汇总统计已保存: {args.output_dir}/crayfish_hgnt_summary.json")

    # 打印均值±标准差格式
    print(f"\n📈 小龙虾 HG-NT 性能指标 (均值 ± 标准差):")
...
# =================================================================
def main():
    parser = argparse.ArgumentParser(description='小龙虾HG-NT模型训练 - 多种子实验')

    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--learning_rate', type=float, default=5e-5)
    parser.add_argument('--patience', type=int, default=15)
    parser.add_argument('--output_dir', type=str, default='Results_Crayfish_HGNT')
    ...
    print("=" * 80)
    print("🔬 小龙虾 HG-NT (Multi-Graph Concat with GCN)")
    print("=" * 80)
    print("\n🔧 训练配置:")
    print(f"  物种: 小龙虾")
    ...
    # 多种子训练
    train_multi_seed(data_dict, args)
