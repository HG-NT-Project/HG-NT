"""
小龙虾基因表达预测 - 消融实验模型套件 V2 (多随机种子重复实验)
统一使用可学习权重融合架构

M1: Base MLP (纯序列基准)
M2: Unified Graph (整图融合 - GCN)
M3: Multi-Graph Weighted Sum (分图加权和 - Softmax归一化可学习权重)
"""
...
# =================================================================
# M2: Unified Graph (整图融合 - 将TF和GCN边合并为一张图)
# =================================================================
...
# =================================================================
# M3: Multi-Graph Weighted Sum (分图加权和 - 可学习权重融合)
# 使用softmax归一化权重，确保两个子图输出的权重和为1
# =================================================================

class ModelM3_WeightedSum(nn.Module):
    """M3: 分图加权和 - 可学习权重融合 (Softmax归一化)
       为TF和GCN子图输出分别设置可学习的权重，使用softmax确保权重和为1，
       然后加权相加，再去做残差连接"""

    def __init__(self, input_dim=2560, hidden_dim=512, dropout=0.3):
        super(ModelM3_WeightedSum, self).__init__()
        ...
        # 可学习的logits，用于softmax计算融合权重
        # 注册为可学习参数，初始值均为0，softmax后得到[0.5, 0.5]
    ...
    def forward(self, x, edge_index=None):
        ...
        weights = {
            ...
            "fusion_gate": tf_weight.item()  # 用TF的权重作为融合门控指标
        }
...
def build_model(model_name, input_dim=2560, dropout=0.3, **kwargs):
    ...
    if model_name == 'm1':
        ...
        return ModelM3_WeightedSum(input_dim=input_dim, dropout=dropout)
    else:
...
class GATDeepCRETrainer:
    ...
    def _extract_unified_subgraph(self, unique_nodes, device):
        """为 M2 模型提取统一的子图（TF+GCN边并集）"""
        unique_nodes_cpu = unique_nodes.cpu()

        # 构建完整的合并边索引（在原始节点索引空间）
        unified_full = None
        if self.tf_edge_index is not None and self.tf_edge_index.numel() > 0:
    ...
    def _prepare_batch_for_model(self, batch):
        ...
        if self.model_name == 'm2':
            # M2: 使用统一的子图
            unified_sub = self._extract_unified_subgraph(unique_nodes, self.device)
            tf_sub = unified_sub
            gcn_sub = unified_sub
        else:
            # 其他模型: 分别提取 TF 和 GCN 子图
            ...
            try:
                ...
                if weights:
                    if 'fusion_gate' in weights:
                        ...
                        epoch_gate_means.append(0.5)

                    if 'aggregation_type' in weights and epoch == 0 and batch_idx == 0:
...
def aggregate_seed_results(all_seed_results):
    ...
    aggregated['seeds'] = [r['seed'] for r in all_seed_results]
...
class CrayfishDeepGATAblation:
    def __init__(self,
                 output_dir='Results_Crayfish_Ablation_V2',
                 batch_size=32,
                 epochs=100,
                 learning_rate=1e-4,
                 patience=15,
    ...
    def _get_model_description(self, model_name):
        descriptions = {
            'm1': 'Base MLP - 纯序列基准，无图信息',
            'm2': 'Unified Graph - 整图融合，TF+GCN边合并',
            'm3': 'Multi-Graph Weighted Sum - 分图加权和 (Softmax归一化可学习权重)'
        }
    ...
    def train_model(self, train_dataset, val_dataset, test_dataset, data_dict, model_name, seed):
        ...
        self._save_predictions(val_gene_ids, val_targets, val_preds,
                               test_gene_ids, test_targets, test_preds, results)
    ...
    def _print_evaluation_summary(self, results):
        print("\n" + "=" * 70)
        print(f"📈 模型性能评估 - {results['model_name'].upper()} (seed={results['seed']})")
        print("=" * 70)
    ...
    def _save_predictions(self, val_gene_ids, val_targets, val_preds,
                          test_gene_ids, test_targets, test_preds, metrics):
        ...
        pred_file = os.path.join(pred_dir, f'crayfish_{metrics["model_name"]}_seed{metrics["seed"]}_predictions.csv')
        all_pred_df.to_csv(pred_file, index=False)

        metrics_file = os.path.join(pred_dir, f'crayfish_{metrics["model_name"]}_seed{metrics["seed"]}_metrics.json')
    ...
    def train(self, model_names=['m1', 'm2', 'm3']):
        print(f"\n{'=' * 80}")
        print(f"🦞 开始训练小龙虾基因表达预测模型 - 消融实验 V2 (多种子重复实验)")
        ...
        print(f"\n📋 消融实验模型架构说明:")
        print(f"  {'=' * 65}")
        print(f"  M1 (MLP):        纯序列基准 - 验证无图时的性能底线")
        print(f"  M2 (Unified):    整图融合 - 验证引入图信息的价值")
        print(f"  M3 (WeightedSum): 分图加权和 (Softmax归一化可学习权重) - 动态权重")
        print(f"  {'=' * 65}")
        print(f"\n💡 M3使用可学习权重自动学习TF和GCN网络的重要性")
        print(f"{'=' * 80}")
    ...
    def _generate_final_report(self):
        ...
        print(f"\n{'=' * 80}")
        print(f"📊 最终实验报告 - 小龙虾消融实验 V2 (多种子重复实验)")
        ...
        print(f"\n📈 模型性能对比 (均值 ± 标准差):")
        print(f"{'-' * 90}")
        print(f"{'模型':<12} {'融合方式':<30} {'Test R²':<18} {'Test Pearson':<18}")
        print(f"{'-' * 90}")
        ...
        for model in order:
            if model in all_aggregated:
                ...
                print(f"{model.upper():<12} {agg_names[model]:<30} "
                ...
                comparison_data.append({
                    ...
                    'test_rmse_mean': agg['test']['rmse']['mean'],
                    'test_rmse_std': agg['test']['rmse']['std']
                })
        ...
        if comparison_data:
            ...
            best_std = next((m['test_pearson_std'] for m in comparison_data if m['model'] == best_model), 0)
            print(f"   Pearson 相关系数: {best_pearson_mean:.4f} ± {best_std:.4f}")
            print(f"{'=' * 80}")
...
# =================================================================
def main():
    parser = argparse.ArgumentParser(description='小龙虾GAT模型 - 消融实验 V2 (多种子重复实验)')

    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--seeds', type=int, nargs='+', default=[42, 123, 789],
                        help='随机种子列表，例如: --seeds 42 123 456 或 --seeds 42')
    ...
    print(f"  ✅ M1: 纯MLP基准")
    print(f"  ✅ M2: 整图融合GCN")
    print(f"  ✅ M3: 分图加权和 (Softmax归一化可学习权重)")
